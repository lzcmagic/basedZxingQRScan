...
public class BezierView extends View {
    ...
    private PointF startPoint, endPoint, controlPoint;//起始 终点 控制点
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        controlPoint.set(mWidth / 2, mHeight / 2 - 200);
        LogUtil.d("s: " + startPoint.x + " " + startPoint.y + "\n" + endPoint.x + " " + endPoint.y + "\n" + controlPoint.x + " " + controlPoint.y);
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        LogUtil.d("ex: " + event.getRawX() + " ey: " + event.getRawY() + "\n" + event.getX() + " " + event.getY());
        controlPoint.set(event.getX(), event.getY());
        invalidate();
        return true;
    }
...
//        canvas.translate(mWidth/2,mHeight/2);
        ...
        LogUtil.d("n: "+startPoint.x+" "+startPoint.y);
        canvas.drawPoint(endPoint.x, endPoint.y, mPaint3);
        canvas.drawPoint(controlPoint.x, controlPoint.y, mPaint3);

        canvas.drawLine(startPoint.x,startPoint.y,controlPoint.x,controlPoint.y,mPaint2);
        canvas.drawLine(endPoint.x,endPoint.y,controlPoint.x,controlPoint.y,mPaint2);
